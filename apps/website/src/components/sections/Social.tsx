import { InstagramPost, SocialSectionProps, TwitterPost } from '@/types'
import Image from '@/components/NextImage'

const SocialSection = ({ instagram, twitter }: SocialSectionProps) => {
    return (
            ...
            <div className="flex flex-1 flex-wrap justify-around my-2">
                <div>
                    ...
                    <div className="grid gap-5 sm:grid-cols-2 grid-cols-1">
                        {instagram.map((post: InstagramPost) => (
                            <a
                                target="_blank"
                                href={post.link}
                                aria-label="Instagram"
                            ...
                            >
                                <Image height="300px" width="300px" src={post.image} alt="" />
                            </a>
                        ))}
                        <div className="flex flex-1 flex-col items-center justify-center text-center font-montserrat text-9xl opacity-25 select-none">
                            ...
                            <p className="w-full text-right tracking-wide">SUN</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="font-openSans text-lg twitter">
                        <ul>
                            {twitter.map((tweet: TwitterPost) => (
                                <li key={tweet.id} className="flex flex-col max-w-md">
                                    <a
                                        target="_blank"
                                        href={tweet.link}
                                        aria-label="Twitter"
